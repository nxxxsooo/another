...
一个项目不等于一个目录。当列出的会话确实分布在多个目录时——worktree、monorepo 的子树——行内会出现目录列，显示相对于项目根的那一段（`.worktrees/delete-undo`、`packages/api`，项目根本身显示 `·`），共有的前缀不会占掉标题的宽度。所有会话都在同一个目录时，这一列不出现。
