...
从 `v0.15.6` 起，主界面和设置页每两秒复测终端尺寸并重新输出完整画面，以修复残影和遗漏的尺寸变化；定期重画不会先清空屏幕。终端支持焦点通知时，切回窗口会立即清屏重绘，失焦期间暂停定期重画；进程恢复时也会立即修复。仍有显示异常时可按 `Ctrl+L` 强制重绘，当前输入、选择和弹窗会保留。重绘不重新扫描会话或刷新索引，刷新数据仍按 `r`。
