...
| OpenCode | `opencode` | `OPC` | `opencode --session <id>`; old isolated stores retain `opencode2` | ✓ | V1 history | V2 sessions | ✓ |
...
**Doubao Work desktop (macOS):** enable it under compatibility adapters in `another setup`. Setup connects only after selection. The adapter lists active Work conversations that have a local desktop session directory, reads their user/assistant text for preview, export, and migration to another agent, and renames through Doubao's native API with readback. It requires an active desktop login, network access, and access to the macOS `Doubao Safe Storage` Keychain item. `DOUBAO_PROFILE` can select a different profile; the default is `~/Library/Application Support/Doubao/Profile 1`. A structured native file-tool path attributes a session to its concrete `~/Doubao/chats/YYYY-MM-DD/new-chat[-N]` workspace; otherwise `~/Doubao/chats` is its PWD. Import into Doubao, native resume, archive, delete, and relocation are not supported. See [provider notes](docs/providers.md#doubao-work-desktop) for data and compatibility limits.
...
The second setup page picks the agent and the language; `Enter` opens a third page for the model. That list comes from the agent's own CLI (`pi --list-models`, `agy models`), typing filters it, the first row leaves the choice to the CLI, and the last row still accepts a name typed by hand. From `v0.15.7`, OpenCode is asked through its own server (`/api/model`) — the catalog its `/models` command shows inside the client, custom providers from your `opencode.json(c)` included — and falls back to `opencode models` on a build without that endpoint. That subcommand was the old source, and it has to resolve a catalog for the directory it was started in: another probes from a throwaway temporary directory, so what came back was the half-resolved state — nothing, then all of models.dev, with your own providers never reached. Claude Code and Qwen Code have no listing subcommand, but their headless control protocols return the catalog available to the current configuration; another sends only control requests, with no prompt or model call. Codex exposes nothing to ask, so it goes straight to typing with the reason shown — a guessed list of model IDs would only fail later, at rename time.
...
OpenCode writes through the released V2 import/API surface by default. An early isolated `opencode2` store retains its matching V2 entry point, while V1 is history-only compatibility. Codex Desktop titles come from its GUI title index rather than guessed injected messages. Pi writes complete assistant records with explicit transport metadata and zero-valued usage for reconstructed history.
