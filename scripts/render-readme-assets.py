...
C = {
    ...
    "purple": "#6B50FF", "pink": "#FF60FF", "green": "#29D398",
    "coral": "#FF6B6B", "cyan": "#62D8FF", "orange": "#FF985A",
}
...
for i, (when, agent, title, path, count, color) in enumerate(rows):
    y = 202 + i * 52
    marker = "›" if i == 0 else ""
    selected = C["pink"] if i == 0 else C["text"]
...
<text x="1070" y="105" fill="{C['muted']}" font-family="{FONT}" font-size="16">target →</text>
<line x1="52" y1="130" x2="1148" y2="130" stroke="{C['line']}" stroke-width="2"/>
{''.join(row_svg)}
<rect x="668" y="154" width="420" height="254" rx="8" fill="{C['bg']}" stroke="{C['purple']}" stroke-width="3"/>
<text x="704" y="198" fill="{C['pink']}" font-family="{FONT}" font-size="19" font-weight="700">Choose target</text>
<text x="704" y="226" fill="{C['muted']}" font-family="{FONT}" font-size="15">Carry this session to another agent.</text>
<text x="708" y="276" fill="{C['coral']}" font-family="{FONT}" font-size="17">› Claude Code</text>
