...
- `docs/assets/another-mark-transparent.png` — the same mark cut off its plate, `1254×1254` RGBA, for surfaces that cannot use a plated square. Reproducible with `scripts/render-mark-transparent.py` from the master. Derived, not a second master: removing the plate dims the phosphor bloom that the plate carries, so the master remains the reference for the bloom.
